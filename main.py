...
#
# If any of the above isn't in place, get_cpu_temp() just returns None
# (shown as "N/A"), same as before - it never crashes the app.
#
# RAM temp still isn't covered - Windows has no built-in API for it and it's
# not something planned to add, so get_ram_temp() stays a stub.
#
...
def get_cpu_temp():
    """Highest current CPU temperature sensor reading, in whole degrees C,
    or None if the DLL/pythonnet/elevation aren't all in place."""
    _init_lhm()
    ...
    try:
        for hardware in _lhm_computer.Hardware:
            if hardware.HardwareType != _lhm_hardware_type.Cpu:
                continue
            hardware.Update()
